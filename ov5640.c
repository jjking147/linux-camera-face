						...
						struct v4l2_mbus_framefmt *mf)
{
	...
	if (!fmt)
	{ /* If not, set it to the default format RGB565 */
		mf->code = ov5640_colour_fmts[0].code;
		mf->colorspace = ov5640_colour_fmts[0].colorspace;
	}
									  ...
									  struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov5640_mode_info_data); i++)
	{
		...
		{
			if (fie->width == ov5640_mode_info_data[i][j].width && fie->height == ov5640_mode_info_data[i][j].height && ov5640_mode_info_data[i][j].init_data_ptr != NULL)
			{
				count++;
			}
